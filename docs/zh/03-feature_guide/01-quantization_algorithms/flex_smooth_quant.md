...
- 钩子�?`[batch, seq, hidden_dim]` 维度上收集激活值统计信息：
  ...
  - **每通道绝对最大�?*：计算激活值的每通道绝对最大值，作为平滑缩放因子的基础�?
...
```yaml
- type: "flex_smooth_quant"           # 固定�?`flex_smooth_quant`，用于指�?Processor�?
  alpha: 0.8                          # 浮点�? 0-1之间，默�?None，通过算法自动搜索最佳alpha，也支持用户自行配置，激活缩放的系数�?
  beta: 0.7                           # 浮点�? 0-1之间，默�?None，通过算法自动搜索最佳beta，也支持用户自行配置，权重缩放的系数�?
...
spec:
  process:
    - type: "flex_smooth_quant"
      alpha: 0.8                          # 激活缩放的权重系数�?-1之间，默认None（自动搜索）�?
...
| 字段�?| 作用 | 说明 |
|--------|------|------|
| type | 处理器类型标�?| 固定�?flex_smooth_quant"，用于标识这是一个灵活平滑量化处理器�?|
...
class FlexSmoothQuantInterface(ABC):
    ...
    def get_adapter_config_for_subgraph(self) -> List[AdapterConfig]:
        """
        返回模型中所有可进行Flex Smooth Quant处理的子图配�?
...
**前置要求�?*
- 模型需要继�?`FlexSmoothQuantInterface` 接口�?
