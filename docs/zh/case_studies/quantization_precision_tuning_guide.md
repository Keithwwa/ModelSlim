...
| 测评结果检�?| 检查量化模型的测评输出，确认无上下文截断、超时等非量化问�?|
| 确定波动范围 | 了解测评数据集本身的精度波动范围，判断当前精度损失是否异�?|
...
| Iterative Smooth | 解决o_proj、down_proj等层因无相邻LayerNorm而无法转移scale的问题。支持对�?非对�?| **优先使用**。运行快，精度较高。超长序列校准集时优先使用。可调整 `alpha` 参数优化 | [iterative_smooth.md](../03-feature_guide/01-quantization_algorithms/iterative_smooth.md) |
| Flex Smooth Quant | 通过二阶段网格搜索自动寻找最优alpha和beta参数，实现更精细的平�?| 当Iterative Smooth不达标、对量化时间不敏感且显存充足时尝试。运行速度较慢 | [flex_smooth_quant.md](../03-feature_guide/01-quantization_algorithms/flex_smooth_quant.md) |
| Flex AWQ SSZ | 结合 AWQ �?SSZ 思想，使用真实量化器评估 MSE 误差，自动搜索最�?alpha | **INT4 低比特量化必�?*。针对低比特权重量化精度敏感的特点，寻找最优参数。精度提升显著，但运行速度较慢 | [flex_awq_ssz.md](../03-feature_guide/01-quantization_algorithms/flex_awq_ssz.md) |
| QuaRot | 通过对权重和激活进行旋转变换，将离群�?分散"到多个通道，平滑分�?| 可与其他算法叠加使用，作为进一步提升精度的备选方案。对�?W4A4 等极端场景效果显�?| [quarot.md](../03-feature_guide/01-quantization_algorithms/quarot.md) |
| KV Smooth | 专门针对 KVCache 中的 Key 离群值抑制，将其迁移�?Query | **KVCache 量化必备**。在不改变注意力打分前提下压�?K 的动态范围，提升生成质量 | [kv_smooth.md](../03-feature_guide/01-quantization_algorithms/kv_smooth.md) |
| LAOS | 协同优化方案。通过 QuaRot �?Iterative Smooth 抑制离群值，配合 AutoRound 优化权重舍入 | **W4A4 极致精度方案**。当前主要适配 Qwen3 稠密系列模型 | [laos.md](../03-feature_guide/01-quantization_algorithms/laos.md) |
...
根据量化对象（权�?激活）和比特数选择合适算法。量化算法选择包括权重量化方法选择和激活量化方法选择两部分�?

#### 权重量化方法对比

| 量化方法 | 特点 | 量化精度 | 量化速度 | 适用场景与建�?|
|---------|------|-----|---|---------------|
...
在量化配置文件中，权重量化通常�?`linear_quant` 处理器的 `qconfig.weight` 部分进行配置（`autoround` 则需使用专门�?`autoround_quant` 处理器）�?
...
在量化配置文件中，激活值量化通常�?`linear_quant` 处理器的 `qconfig.act` 部分进行配置�?
...
3. **对称�?(`symmetric`)**：激活值量化通常设置为`false`（非对称量化），以更好地适应非零中心的数据分布�?
...
| 调整策略 | 具体操作 | 优化目的 |
|---------|---------|---------|
| 增加数据�?| 适当增加数据量（建议10-50条样本） | 提升量化参数估计的准确�?|
| 匹配应用场景 | 使用与模型应用场景匹配的数据（如中文模型用中文数据，代码模型用代码数据） | 使校准数据更贴近实际应用场景 |
| 平衡数据分布 | 从多个数据集中抽取样本混合，平衡数据分布 | 提升数据分布的多样性和均衡�?|
| 删除异常数据 | 删除导致精度显著下降的校准数�?| 减少异常样本对量化参数的干扰 |
| 加入badcase | 加入模型在该数据集上的badcase数据，以更好地反映模型真实输入分�?| 帮助量化模型学习困难样本，提升精�?|

#### 总结建议

1. **数据�?*：建议使�?0-50条样本，过少可能无法充分估计量化参数，过多则可能增加量化时间�?
...
spec:
  process:                     # 处理器列�?
    - type: "linear_quant"     # 处理器类型：线性层量化
      ...
      exclude: ["*model.layers.*.mlp.down_proj*"] # 排除的层。默认：[]。此处回退所有mlp.down_proj�?
```
