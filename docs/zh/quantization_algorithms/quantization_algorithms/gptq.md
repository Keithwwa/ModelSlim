...
2. **基于二阶信息的误差修�?*：利用Hessian矩阵评估权重量化对输出的影响，并动态调整未量化权重以补偿误差�?
3. **分块量化**：将权重划分成多个块，减少计算复杂度�?
4. **惰性批量更�?*：在更新未量化权重时，延迟更新，将多个更新合并执行，减少计算开销�?

### 实现

算法实现�?[`msmodelslim/core/quantizer/impl/gptq.py`](../../../../msmodelslim/core/quantizer/impl/gptq.py) 中：
  - `per_channel`实现类：`WeightPerChannelGPTQ`
  - `per_group`实现类：`WeightPerGroupGPTQ`

## 适用要求

- **高精度需�?*：适用于对精度要求较高的模型量化场景�?
- **计算成本**：GPTQ算法需要计算激活值的hessian矩阵和矩阵分解，且对权重逐列量化，量化速度较慢，计算成本较大�?
- **使用限制**�?
    - 目前支持int8场景�?per_channel/per_group 对称与非对称量化�?
    - 当前暂不支持int4场景�?per_channel/per_group 对称与非对称量化（后续将支持）�?
    - per_tensor量化粒度暂不支持�?
    - 权重必须�?D张量�?
    ...
    - type: "linear_quant" 
      qconfig:
        weight:
          ...
          ext:                   # 可选，扩展参数
            percdamp: 0.01       # 可选，阻尼系数，默认�?.01
            block_size: 128      # 可选，分块大小，默认�?28
```
