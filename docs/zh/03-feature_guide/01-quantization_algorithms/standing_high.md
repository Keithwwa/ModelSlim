...
Standing High 算法的核心思想�?摸高"：算法首先使用配置文件中的初始配置进行评估，如果满足精度要求则直接返回。否则，使用二分搜索快速定位满足精度要求的最小回退级别，然后通过摸高过程逐步减少回退层数，结合不同的离群值抑制策略，寻找最优配置。算法采用自适应步长机制，在找到满足要求的配置时加快搜索速度�?
...
算法使用配置文件中的初始配置（`template`），直接生成量化配置并评估。初始配置基�?`template` 中的量化配置，如果该配置满足精度要求，算法直接返回，结束调优�?
...
2. **遍历离群值抑制策�?*：对于每个回退级别，算法会遍历配置文件中定义的所有离群值抑制策略（`anti_outlier_strategies`），尝试不同的策略组合�?
...
5. **终止条件**：当步长已经�?，且所有离群值抑制策略都不满足要求时，算法返回之前找到的最佳配置�?
...
### anti_outlier_strategies - 离群值抑制策略列�?

离群值抑制策略列表，每个元素是一个处理器配置列表。算法会遍历这些策略组合，寻找满足精度要求的配置�?

**配置说明**�?
- 可以配置空列�?`[]`，表示不使用离群值抑制策�?
- 可以配置单个离群值抑制策略，�?`iter_smooth`
...
anti_outlier_strategies:
  - [ ]  # 不使用离群值抑制策�?
  - - type: "iter_smooth"  # iter smooth 离群值抑制策�?
      alpha: 0.5
  - - type: "iter_smooth"  # 使用混合策略
      alpha: 0.5
    - type: "quarot"  # QuaRot 离群值抑制策�?
```
...
**配置协议**: template 字段的配置协议与一键量化配置协议中�?`spec` 字段保持一致，详细配置说明请参考[一键量化配置协议说明](../功能指南/一键量�?配置协议说明.md)�?
...
| 字段�?| 作用 | 类型 | 说明 |
|--------|------|------|------|
| w_bit | 权重量化位数 | int | 权重量化的位数，�?表示8bit量化 |
| a_bit | 激活值量化位�?| int | 激活值量化的位数，如8表示8bit量化 |
| is_sparse | 是否稀�?| bool | 是否为稀疏量�?|
...
2. **找到最优配�?*：在摸高过程中，如果当前回退级别已经�?（不设置任何回退层，所有层都量化），且所有离群值抑制策略都已尝试，算法返回找到的最佳配置�?

3. **无法进一步优�?*：如果步长已经是1，且所有离群值抑制策略都不满足要求，算法返回之前找到的最佳配置�?

## 注意事项

1. **模板配置要求**：`template` 中必须包含且仅包含一�?`linear_quant` 处理器，否则会抛出配置错误�?

2. **离群值抑制策�?*：至少需要配置一个离群值抑制策略（可以是空列表，表示不使用离群值抑制）�?

3. **回退级别**：回退级别由量化回退层选择器根据模型结构和校准数据自动计算，级别越高，回退到浮点的层越多。回退级别�?表示所有层都量化，不回退任何层�?

4. **推理引擎支持**：需要注意推理引擎对任意回退的支持情况。一般情况下，单算子模式�?vLLM-Ascend 支持任意回退，但使用混合算子时可能不支持任意回退。在使用 Standing High 算法时，请确保配置的推理引擎（如 vLLM-Ascend）能够支持量化回退功能�?
