...
注意：自动恢复功能要求 `save_path` 与之前中断时的路径一致，且 `save_path/history` 目录中存在有效的历史精度缓存。如果历史精度缓存不存在或无效，系统将从头开始调优。
...
3. 调优过程会自动尝试不同的量化配置，每次迭代都会获取量化配置的评估结果（优先从历史精度缓存中复用，如果不存在则进行实际评估），直到找到满足所有精度需求的量化方案。
...
3. **检测历史记录**：自动检测 `save_path/history` 目录中是否存在历史精度缓存。如果检测到精度缓存，系统会在后续迭代中复用已评估过的量化配置的评估结果。
...
4. **开始迭代调优**：记录调优开始时间和超时时间，进入迭代循环：
   ...
   - **尝试恢复历史**：如果存在历史精度缓存，系统会尝试从精度缓存中恢复当前迭代的评估结果。系统会在精度缓存中查找匹配的评估结果，如果找到匹配的评估结果，则直接使用历史评估结果，跳过量化、服务化启动、预检查和评估步骤。
