...
- 问题：传�?W8A8 静态量化采用静态的激活量化参数，在长上下文或分布漂移场景中易产生较大量化误差，需要回退大量层才能控制精度损失，却因此损失性能收益�?
...
- 该算法为线性层量化算法中的激活值量化算法，线性层�?torch.nn.Linear 实现即满足算法需�?
...
- 量化校准：[`msmodelslim/core/quantizer/impl/minmax.py`](../../../../msmodelslim/core/quantizer/impl/minmax.py) 中的
  `ActPDMixMinmax`
- 量化模式 IR：[`msmodelslim/ir/w8a8_pdmix.py`](../../../../msmodelslim/ir/w8a8_pdmix.py) 中的
...
### 昇腾AI处理器支持情�?
- 当前仅支持Atlas A2训练、推理、Atlas A3训练、推理系列产品推理部�?

### 使用说明

PDMIX 量化算法通过 ModelSlimV1 �?YAML 配置文件使用�?

```yaml
spec:
  process:
    - type: "linear_quant"     # 线性层量化模式处理�?
      qconfig:
        act: # 激活值量化配�?
          scope: "pd_mix"      # prefilling: per_token；decoding: per_tensor
          dtype: "int8"        # 暂时仅支�?INT8
          symmetric: false     # PDMIX 量化总体为非对称
          method: "minmax"     # 暂时仅支�?MinMax 算法
        weight: # 权重量化配置   
          scope: "per_channel" # 暂时仅支持搭配权�?per-channel 量化
          dtype: "int8"        # 暂时仅支持搭配权�?INT8 量化
          symmetric: true      # 仅支持搭配权重对称量�?
