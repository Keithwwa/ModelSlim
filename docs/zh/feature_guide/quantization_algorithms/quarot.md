...
1. **核心概念**�?
   QuaRot 采用正交旋转矩阵**（如 Hadamard 矩阵）对模型权重和激活值进行变换。正交矩阵满�?Q × Qᵀ = I 的关键性质，确保变换前后模型数学等价�?
2. **旋转变换操作�?*
   对权重矩�?W 应用变换：`W' = Qᵀ × W`�?
   对激活�?X 应用变换：`X' = X × Q`�?
   变换保持计算等价：`X' × W' = (X × Q) × (Qᵀ × W) = X × W`�?
3. **计算不变�?*�?
   旋转变换保持 Transformer 各层的输�?输出映射关系不变。即使层间包�?RMSNorm 操作，因 `RMSNorm(X) = RMSNorm(X × Qᵀ) × Q`
   ，计算不变性依然成立�?
4. **优化效果**�?
...
- 调用`adapter.get_ln_fuse_map()`获取LayerNorm与Linear层的融合映射，返回`(pre_run_fused_ln, fused_map)`，其�?
...
- **张量并行限制**：若在配置中启用了在线旋转，在使用推理引擎以TP并行的方式进行部署时，需要保证`tp_size`�?的幂，并且`tp_size`需要小于等于QuaRot的配置参数`max_tp_size`，否则必然导致精度异常�?
...
```yaml
- type: "quarot"                         # 固定�?`quarot`，用于指�?Processor 类型�?
  online: False                          # 控制是否启用在线旋转，默认为 False�?
  block_size: -1                         # 整数, 取值范围为-1或大�?�?的幂，表示启用块对角矩阵时每个块的大小，若为-1，表示不进行块对角矩阵处理�?
  max_tp_size: 4                         # 整数，默认为4，该配置项仅在启用在线旋转时生效。最大张量并行大小，必须大于0且为2的幂或等�?，拉起模型时设置的tp值必�?=max_tp_size�?
...
spec:
  process:
    - type: "quarot"
      online: False                      # 控制是否启用在线旋转，默认为 False�?
      block_size: -1                     # 旋转矩阵启用块对角矩阵时每个块的大小, 取值范围为-1�?的幂次方，如果大�?必须�?的幂，若�?1，表示不进行块对角矩阵处�?
      max_tp_size: 4                     # 最大张量并行大小，默认�?，仅在启用在线旋转时生效，必须大�?且为2的幂，拉起模型时设置的tp值必�?=max_tp_size
      down_proj_online_layers: [ ]        # 用于指定哪些层的down_proj使用在线旋转，默认为�?
```
...
| 字段�?                    | 作用             | 类型           | 说明                                          | 默认�?       |
|-------------------------|----------------|--------------|---------------------------------------------|------------|
| type                    | 处理器类型标�?       | `string`     | 固定值，用于标识这是一个QuaRot量化处理�?                    | `"quarot"` |
| online                  | 在线旋转开�?        | `bool`       | 是否启用在线旋转，True表示使用在线旋转，False表示不使�?           | `False`    |
| block_size              | 旋转矩阵的对角块大小     | `int`        | 旋转矩阵的对角块大小，取值范围为-1或大�?�?的幂，若�?1表示不进行块对角矩阵处理 | `-1`       |
| max_tp_size             | 最大张量并行大�?      | `int`        | 该配置项仅在启用在线旋转时生效，最大张量并行大小，必须大于0且为2的幂或等�?     | `4`        |
| down_proj_online_layers | 指定使用在线旋转的down�?| `array[int]` | 用于指定哪些层的down_proj使用在线旋转，类型为由层索引组成的列�?       | `[]`       |
...
class RotatePair:
    """旋转对数据类，包含左旋转和右旋转的映�?""
    left_rot: Dict[str, Any]  # 左旋转映射：{模块�? 旋转矩阵}
    right_rot: Dict[str, Any]  # 右旋转映射：{模块�? 旋转矩阵}
```
...
class QuaRotInterface:
    ...
    def get_ln_fuse_map(self) -> Tuple[Dict[str, List[str]], Dict[str, List[str]]]:
        ...
        Returns:
            包含两个字典的元组：
            - pre_run_fused_ln (Dict[str, List[str]]): pre_run阶段的融合映�?
            - fused_map (Dict[str, List[str]]): preprocess阶段的融合映�?
            字典的key为LayerNorm层名称，value为需要融合的Linear层名称列�?
        """
        ...
...
class LAOSOnlineRotationInterface:
    ...
    def get_layer_wise_ov_pair(self, decoder_module: nn.Module) -> Dict[nn.Module, nn.Module]:
        """
        获取单个decoder层对应的o_proj和v_proj映射�?
    ...
    def get_layer_wise_up_down_pair(self, decoder_module: nn.Module) -> Dict[nn.Module, nn.Module]:
        """
        获取单个decoder层对应的up_proj和down_proj映射�?
...
- **解决方案**：请先确定指定维度的 Hadamard 矩阵存在，参�?`msmodelslim/processor/quarot/hadamard_txt` 添加特定维度的矩阵，并且�?
