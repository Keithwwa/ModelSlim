...
- 算法�?`msmodelslim/core/quantizer/impl/histogram.py` �?`msmodelslim/core/observer/histogram.py` 中实现，处理流程�?步�?

1. **直方图统�?*�?
   - 将输入张量的值域划分为固定数量的bins（默�?048）�?
...
- type: "linear_quant" 
  qconfig:
   act:
     ...
     method: "histogram" # 配置�?histogram", 即启用直方图激活值量�?
   weight:
     scope: "per_channel"
     dtype: "int8" 
     symmetric: true
...
| method | 量化方法 | `"histogram"` | histogram: 直方图量�?| `"histogram"` |
...
class HistogramObserver(TorchHistogramObserver):
    def __init__(self, config: HistogramObserverConfig):
        ...
        self.clip_min = None
        self.clip_max = None   
        self.upsample_rate = 16  # 上采样率，减少量化误�?
```
...
2. **update方法**�?
   ...
   def update(self, x: torch.Tensor, sync: bool = False, group: Optional[dist.ProcessGroup] = None):
       """
       更新直方图，并进行截断值搜索，保存最佳的量化截断�?
       
       主要步骤�?
       1. 输入验证：检查张量有效性，过滤NaN和无穷�?
       2. 直方图更新：调用父类forward方法更新直方图统�?
       3. 参数搜索：执行非线性参数搜索，找到最优截断区�?
...
4. **非线性参数搜�?*�?
   ...
   def _non_linear_param_search(self) -> tuple[torch.Tensor, torch.Tensor]:
       ...
       3. 边界调整：根据移动步长，决定移动左边界还是右边界：选择单次移动长度更长（即分布更稀疏）的一�?
       4. 早停条件：量化误差不再改善或边界越界时停�?
       5. 返回结果：最优的start_bin和end_bin对应的截断�?
       """
   ```
...
- type: "linear_quant" 
  qconfig:
   ...
   weight:
     ...
     method: "minmax" # 不支持直方图权重量化，此处不应配置为"histogram"
```
- 排查对应的quantizer在初始化时是否存在AutoActQuantizer。可以根据配置yaml中qconfig对应�?type查看名字，在`msmodelslim\core\quantizer`查看对应的代码�?
```python
class LinearQuantizer(nn.Module):
    ...
    def __init__(self, config: LinearQConfig, *args, **kwargs):
        ...
        self.config = config
        self.input_quantizer = AutoActQuantizer.from_config(config.act)  # 支持激活值量�?
