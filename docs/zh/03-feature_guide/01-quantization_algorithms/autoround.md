...
- **来源**：Intel 开发的一种基�?SignSGD 的低比特量化方法�?
...
其中 s 是缩放因子，zp 是零点，n �?m 是量化后的上下界�?

AutoRound 在此基础之上引入了可学习的舍入偏�?V 和可选的缩放因子调整参数 α �?β�?

`Ŵ = s × clip(⌊W/s + zp + V�? n, m)`，`s = (max(W) × α - min(W) × β) / (2^bit - 1)`

其中 V 用于控制舍入的方向，α �?β 用于调整缩放因子的范围�?
...
2. **参数初始�?*：对缩放因子、舍入偏移量进行初始化，并将其设置为可训练参数引入量化过程中
3. **量化重构**：执行量�?反量化操作，进行前向传播得到量化结果
4. **损失计算**：对比量化输出与浮点输出的差异，计算重构损失
5. **参数更新**：通过SignSGD优化器更新缩放因子和舍入偏移�?
6. **迭代优化**：重复步�?-5直至满足收敛条件或达到最大迭代次�?
7. **最终量�?*：应用迭代训练后的最优参数得到最终量化权�?

### 实现

- 算法�?`msmodelslim/processor/quant/autoround.py` 中实现，核心类为 `AutoroundQuantProcessor`�?
  1. **初始化阶�?*�?

     * 层配置初始化：读取量化配置，并为每个网络层分配对应的量化配置方案
     * 参数预分配：初始化浮点输出、量化输出和最佳参�?
  2. **pre_run阶段**�?
  ...
  3. **preprocess阶段**：（逐层循环执行�?
     ...
     * 计算图构建：建立包含量化和反量化操作的可微计算图，支持梯度反向传�?
  4. **process阶段**：（逐层循环执行�?

     * 训练器初始化：设置学习率、迭代次数等参数，配置SignSGD优化�?
     * 输入输出配置：使用上一层量化输出作为当前层输入，以浮点输出和量化输出的差距作为优化目标
     * 参数优化：通过多次迭代更新缩放因子和舍入偏移，最小化重构误差
     * 收敛监测：实时监测损失变化，达到收敛阈值或最大迭代次数时停止优化，得到最优参�?
  5. **postprocess阶段**：（逐层循环执行�?

     * 参数应用：将优化后的量化参数应用于对应层的权重量�?
     * 解除封装：移除线性层的包装，恢复原始网络结构
     * 前向传播：执行当前层量化后的前向传播，作为下一层的输入
  6. **post_run阶段**�?

     * 清理工作：移除所有模块的临时属性，完成量化流程的最终清理工�?

## 适用要求

- **低比特量�?*：适合极低比特量化场景中的4比特量化�?
- **高精度需�?*：在低比特条件下仍能保持较高的模型精度�?
...
```yaml
# AutoRound支持混合量化，即对不同的层使用不同的量化配置，这里以 W8A8 �?W4A4 混合量化为例
# W8A8 动态量化配�?
default_w8a8_dynamic: &default_w8a8_dynamic
  weight:
    ...
    method: "autoround"       # 权重量化方法：AutoRound算法，即包含参数训练的权重量�?
    ext:
      group_size: 256         # 量化组大小，分组将在待量化nn.Linear的input_features维度进行，该值必须能够被其整�?
      scale_dtype: "bfloat16" # 缩放因子数据类型
  act:
    scope: "per_token"        # 激活值量化范�?
    dtype: "int8"             # 激活值量化数据类�?
    symmetric: True           # 是否启用对称量化
...
spec:
  process:
    - type: "autoround_quant"     # 固定�?`autoround_quant`，用于指�?Processor 类型�?
      iters: 400                  # 优化迭代次数
      enable_minmax_tuning: True  # 是否启用最小最大值调�?
      enable_round_tuning: True   # 是否启用舍入调优
      strategies:
        # 策略1：除 up_proj、gate_proj �?o_proj 层外，其余层均应�?W8A8 量化�?
...
| 字段�?| 作用 | 类型 | 说明 | 默认�?|
|--------|------|------|------|--------|
| type | 处理器类型标�?| `string` | 固定值，用于标识这是一个AutoRound量化处理�?| `"autoround_quant"` |
| iters | 优化迭代次数 | `int` | 迭代次数，必须大�?，影响优化效果和计算时间 | `10` |
| enable_minmax_tuning | 是否启用最小最大值调�?| `bool` | 是否启用最小最大值调优，True表示启用，False表示不启�?| `True` |
| enable_round_tuning | 是否启用舍入调优 | `bool` | 是否启用舍入调优，True表示启用，False表示不启�?| `True` |
...
| 字段�?| 作用 | 类型 | 说明 | 示例�?|
|--------|------|------|------|--------|
| qconfig | 量化配置参数 | `object` | 包含激活值量化和权重量化的详细配�?| [激活值配置](#qconfigact-激活值量化配�?、[权重量化配置](#qconfigweight-权重量化配置) |
...
| method | 量化方法 | `"autoround"` | 权重量化方法：AutoRound算法，即包含参数训练的权重量�?| `"autoround"` |
| ext | 扩展配置 | `object` | 包含AutoRound特有的配置参�?| [见下方详细配置](#ext-autoround扩展配置) |
...
| 参数�?| 作用 | 类型 | 说明 | 示例�?|
|--------|------|------|------|--------|
| group_size | 量化组大�?| `int` | 分组量化的大小，必须能被待量化nn.Linear层的input_features维度整除 | `256` |
...
class QuantStrategyConfig(BaseModel):
    ...
    include: List[str] = Field(default_factory=lambda: ["*"], description="包含的模块名�?)
    exclude: List[str] = Field(default_factory=list, description="排除的模块名�?)

# AutoroundProcess处理器配�?
class AutoroundProcessorConfig(AutoProcessorConfig):
    type: Literal["autoround_quant"] = Field(default="autoround_quant", description="处理器类型标�?)
    iters: int = Field(default=10, gt=0, description="迭代次数，必须大�?")
    enable_minmax_tuning: bool = Field(default=True, description="是否启用最小最大值调�?)
...
- **前置要求**�?
  ...
  - 硬件环境：优化过程需要额外的内存来存储中间变量和梯度信息，具体支持的硬件产品系列请参�?[硬件产品支持](#硬件产品支持)�?

- **步骤**�?

  1. 在配置文件中定义量化策略，支持针对不同的层使用不同的量化策略�?
  2. 在配置文件中使用`"autoround_quant"`指定autoround处理器，并且配置相关参数�?
  3. 如需使用自定义校准集，可参�?`msmodelslim/lab_calib`添加数据集，并在配置文件中指定数据集名称�?
...
- **解决方案**�?
  ...
  - 常见的`input_features`维度包括�?096�?192�?1008�?
  - 推荐的`group_size`值：128�?56�?12等，这些值通常能够被大多数层的`input_features`整除

### 4. 层匹配告�?

层匹配告警的处理机制与LinearQuantProcess相同。当`include/exclude`模式未匹配到任何层时，工具会进行告警。详细的常见匹配失败原因和排查步骤请参�?[LinearQuantProcess层匹配告警](../02-one_click_quantization_v1/linear_quant.md#层匹配告�?�
