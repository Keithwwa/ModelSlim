...
2. **最小二乘法**：使用最小二乘法计算当前最优的 scale �?offset�?
3. **贪心更新**：只保留能改善量化误差的参数�?
4. **收敛判断**：通过相对和绝对误差变化来判断收敛�?

算法流程�?
```
1. 使用 MinMax 观察器初始量化参�?scale �?offset�?
2. 通过最小二乘法计算当前最优的 scale �?offset�?
...
- 算法�?`msmodelslim/core/quantizer/impl/ssz.py` 中实现，核心函数�?`ssz_calculate_qparam`�?
#### 1. 初始化阶�?
    - 使用MinMax观察器计算权重的统计信息（min/max值）�?
    - 基于统计信息计算初始的量化参数（scale和offset）�?
#### 2. 迭代优化阶段
    - 对称量化：offset固定�?，只优化scale�?
...
print(f"量化后权重形�? {q_storage.value.shape}")
...
```python
SCALE_SEARCH_ITER_NUM = 20                    # 最大迭代次�?
SCALE_SEARCH_CONVERGE_THRESHOLD = 1e-10       # 收敛阈�?
SCALE_SEARCH_MIN_SCALE = 1e-5                 # 最小缩放因�?
```
