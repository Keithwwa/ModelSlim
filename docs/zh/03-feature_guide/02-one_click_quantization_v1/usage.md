...
2. 若最佳实践库中未搜寻到最佳配置，系统则会向用户询问是否采用默认配置，即使�?`msmodelslim/lab_practice/default/default.yaml` 实施量化�?
3. 如果需要打印量化运行日志，可通过以下环境变量进行设置�?

| 环境变量                  | 解释        | 是否可�?| 范围             |
|-----------------------|-----------|------|----------------|
| MSMODELSLIM_LOG_LEVEL | 打印同级及以上日�?| 可�?  | INFO(默认),DEBUG |
...
使用一键量化功能量�?Qwen2.5-7B-Instruct 模型，量化方式采�?w8a8�?
...
其中�?
- `${MODEL_PATH}` �?Qwen2.5-7B-Instruct 原始浮点权重路径
- `${SAVE_PATH}` 为用户自定义的量化权重保存路�?
...
| 算法名称 | 处理器类�?| 支持状�?| 说明 |
|----------|----------|---------|------|
| Iterative Smooth | iter_smooth | �?支持 | 完全支持分布式执�?|
| Flex Smooth Quant | flex_smooth | �?支持 | 完全支持分布式执�?|

**量化算法**

| 算法名称 | 量化方法 | 支持状�?| 说明 |
|----------|---------|---------|------|
| MinMax | minmax | �?支持 | 完全支持分布式执�?|
| SSZ | ssz | �?支持 | 完全支持分布式执�?|
...
一键量化配置协议采用分层结构设计思想，通过YAML把整条量化流水线抽象成配置：使用的量化服务版本、流水线类型、量化处理方式、保存策略以及量化校准集等。开发者只关心“策略和流程”，无需�?Python 里硬编码这些细节�?
...
```yaml
apiversion: "modelslim_v1"   # 协议版本：用于选择后端量化服务的版�?
spec:                         # 具体的量化服务配置字�?
  runner: "auto"              # 量化调度器类型。默认：auto
  process: [ ]                # 处理器配置列表：按顺序执行每个处理器
  save: [ ]                   # 保存器配置列表：定义量化结果的保存方�?
  dataset: "mix_calib.jsonl"  # 校准数据集配置：将会从lab_calib目录下匹配使用的校准�?
```
...
| 参数           | 可�?必�?| 说明                                                                                                                                                                  | 作用                                |
|--------------|-------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|
| apiversion   | 必�?   | 1. 当前支持列表：`"modelslim_v0"`、`"modelslim_v1"`、`"multimodal_vlm_modelslim_v1"`、`"multimodal_sd_modelslim_v1"`�?br> 2. 工具根据此字段选择对应的量化服务后端�?br> 3. 不同版本的量化服务可能有不同的配置字段和参数要求�?| 用于选择后端量化服务的版本，不同的量化服务有着不同的具体配置协议�?|
| spec         | 必�?   | 1. **流水线定�?*：指定量化处理的流水线类型�?br> 2. **处理器配�?*：定义各种量化处理器的参数�?br>3. **保存策略**：指定量化结果的保存方式和格�?<br>4. **数据集配�?*：指定校准数据集                            | 具体的量化服务配置字段，包含量化策略、处理流程和保存方式等所有具体参数�?                                |

**协议版本维护策略**�?

| 协议版本         | 维护策略 | 状�? |
|--------------|------|-----|
| modelslim_v0 | 即将废弃 | 不推�?|
...
| KVCache Quant | 量化 | [KVCache Quant 配置示例](../01-quantization_algorithms/kvcache_quant.md#yaml配置示例) | [KVCache Quant 配置字段详解](../01-quantization_algorithms/kvcache_quant.md#yaml配置字段详解) |
| FA3 Quant | 量化 | [FA3 Quant 配置示例](../01-quantization_algorithms/fa3_quant.md#yaml配置示例) | [FA3 Quant 配置字段详解](../01-quantization_algorithms/fa3_quant.md#yaml配置字段详解) |
| Float Sparse | 量化 | [Float Sparse 配置示例](../01-quantization_algorithms/float_sparse.md#yaml配置示例) | [Float Sparse 配置字段详解](../01-quantization_algorithms/float_sparse.md#yaml配置字段详解) |
| AutoRound | 量化 | [AutoRound 配置示例](../01-quantization_algorithms/autoround.md#yaml配置示例) | [AutoRound 配置字段详解](../01-quantization_algorithms/autoround.md#yaml配置字段详解) |
| LAOS (W4A4方案) | 综合方案 | [LAOS 配置示例](../01-quantization_algorithms/laos.md#yaml配置示例) | [LAOS 配置字段详解](../01-quantization_algorithms/laos.md#yaml配置字段详解) |
...
| 字段�?| 作用 | 说明 |
|--------|------|------|
| type | 保存器类型标�?| 固定�?ascendv1_saver"，用于标识这是一个Ascend格式保存�?|
...
在一键量化中，通过 `qconfig.act.scope` 字段来区�?**静态量�?* �?**动态量�?*�?
- **静态量�?(`per_tensor`)**：在校准阶段统计并固定量化参数，推理时不再计算。特点是**推理性能最�?*，硬件兼容性好�?
- **动态量�?(`per_token`)**：在推理时为每个 token 动态计算量化参数。特点是**精度更高**，能有效应对激活值中的离群值分布�?
...
spec:                          # 规格定义
  ...
  save:                        # 保存器配置列�?
    - type: "ascendv1_saver"   # 使用标准 ascendv1 格式保存
      part_file_size: 4        # 权重分片大小�?GB（建议大模型开启）
```
...
default_w8a8_dynamic: &default_w8a8_dynamic
  ...
  weight:                      # 权重量化配置
    ...
    method: "minmax"           # 量化算法：minmax
                               #
# 定义 W8A8 静态量化配置模�?
default_w8a8: &default_w8a8    # 静态量化模板定�?
  act:                         # 激活值配�?
    scope: "per_tensor"        # 静态量化标识：整个张量共用量化参数
    dtype: "int8"              # 数据类型：int8
    symmetric: False           # 非对称量化：False（静态量化常用）
    ...
    - type: "group"            # 使用组合处理器，支持对不同层应用不同配置
      configs:                 # 组合内的子处理器配置列表
        - type: "linear_quant" # 线性层量化子处理器 1
          qconfig: *default_w8a8 # 引用静态量化模板：�?Attention 层使用静态量化以优化性能
          include: ["*self_attn*"] # 匹配包含 self_attn 的层
                               #
        - type: "linear_quant" # 线性层量化子处理器 2
          qconfig: *default_w8a8_dynamic # 引用动态量化模板：�?MLP 层使用动态量化以保障精度
          include: ["*mlp*"]   # 匹配包含 mlp 的层
          exclude: ["*gate"]   # 从上述匹配中排除门控�?
                               #
  save:                        # 保存器配置列�?
    - type: "ascendv1_saver"   # 使用标准 ascendv1 格式保存
      part_file_size: 4        # 权重分片大小�?GB（建议大模型开启）
...
- **多模态支�?*：针对文本到视频等任务的模型优化�?
- **动态静态量�?*：支持动态、静态激活值量化，适应不同的输入场景�?
...
- 支持`dump_config`配置，用于校准数据的捕获和存�?
- 支持`model_config`配置，包含模型加载和推理的相关参�?

#### runner - 量化调度器类�?

当前多模态生成模型考虑到显存占用问题，默认且仅支持layer_wise（逐层量化）形式。runner默认无需配置，配置为�?layer_wise'值时，会警告提示并自动转换为layer_wise（逐层量化）形式�?

#### process - 处理器配置字�?

此配置字段与 modelslim_v1 保持一致，参考[modelslim_v1 配置详解/process - 处理器配置字段](#process---处理器配置字�?
...
spec:
  save:
    - type: "mindie_format_saver"   # 保存器类型：保存为MindIE-SD格式
      part_file_size: 0            # 分片文件大小（GB），0表示不分�?
```
...
| 字段�?| 作用 | 说明 |
|--------|------|------|
| type | 保存器类型标�?| 固定�?mindie_format_saver"，用于标识这是一个MindIE-SD格式保存�?|
| part_file_size | 分片文件大小 | 分片文件的大小，单位为GB�?表示不分�?|

#### multimodal_sd_config - 多模态生成特有配置字�?
...
spec:
  multimodal_sd_config:
    dump_config:
      ...
      dump_data_dir: ""            # 校准数据保存目录。空字符串时自动处理为权重保存路�?
```
...
| 字段�?| 作用 | 说明                                                                                                            | 可选�?|
|--------|------|---------------------------------------------------------------------------------------------------------------|--------|
| capture_mode | 数据捕获模式 | 指定如何捕获模型的输入数�?                                                                                                | 当前仅支�?args"，其他模式待后续扩展 |
| dump_data_dir | 校准数据目录 | 指定校准数据的检索和保存路径，空字符串时自动处理为使用权重保存路径。指定路径存在calib_data.pth时，直接加载作为校准数据，当calib_data.pth文件不存在时程序自动通过dump机制保存并加载校准数�?| 字符串路�?|

**捕获模式说明**:

- **args**: 捕获位置参数，适用于大多数多模态生成模�?

##### model_config - 模型加载与推理配�?
...
spec:
  multimodal_sd_config:
    model_config:
      prompt: "A stylish woman walks down a Tokyo street..." # 校准提示�?
      offload_model: True          # 是否在推理后卸载模型到CPU
      frame_num: 121               # 视频生成的帧�?
      task: "t2v-14B"              # 任务类型
...
| 字段�?| 作用 | 说明 | 可选�?|
|--------|------|------|--------|
| prompt | 校准提示�?| 用于生成校准数据的文本描�?| 字符�?|
| offload_model | 模型卸载 | 是否在推理后卸载模型到CPU，值为True时开�?| True/False |
| frame_num | 生成帧数 | 视频生成的帧�?| 大于0的整�?|
| task | 任务类型 | 指定模型任务类型�?t2v-14B"表示14B模型的文本生成视频任务�?t2v-1.3B"表示1.3B模型的文本生成视频任�?| "t2v-14B", "t2v-1.3B" |
| size | 生成尺寸 | 视频或图像的尺寸规格 | "1280\*720", "832\*480" |
| sample_steps | 采样步数 | 扩散模型的采样步�?| 大于0的整�?|

#### dataset - 校准数据集配�?

不支持通过dataset字段配置校准数据集。由于多模态生成模型的量化校准数据处理方式与大语言模型存在明显差异，将通过识别指定dump_data_dir目录中是否存在名�?calib_data.pth"的校准数据，选择直接加载或自动获取并保存两种方式。详见[dump_config - 校准数据捕获配置](#dump_config---校准数据捕获配置)
...
- **多模态VLM支持**：针对图�?文本多模态理解模型优化�?
- **逐层处理**：采用逐层量化策略，显著减少大模型量化时的显存消耗�?
- **多种数据集格�?*：支持图像目录和通过JSON/JSONL格式自定义文本prompt的多模态校准数据集�?

**适用模型类型**:

- Qwen3-VL-MoE系列：Qwen3-VL-235B-A22B、Qwen3-VL-30B-A3B等多模态模�?
- 其他多模态VLM模型：待后续逐步支持
...
- 默认使用layer_wise（逐层量化）模式，针对大规模多模态模型优�?
- 继承modelslim_v1的所有处理器和保存器配置

#### runner - 量化调度器类�?

当前多模态VLM模型考虑到显存占用问题，默认且仅支持layer_wise（逐层量化）形式。runner默认无需配置，配置为�?layer_wise'字段时，会警告提示并自动转换为layer_wise（逐层量化）形式�?
...
**类型**: `string`
**默认�?*: `"Describe this image in detail."`
**限制**：不能使用空字符串作为文本prompt，当dataset字段配置为包含JSON/JSONL文件（用于描述每个图像的自定义文本prompt）的图像目录时，此字段失效�?

#### save - 保存器配置字�?

此配置字段与 modelslim_v1 保持一致，参考[modelslim_v1 配置详解/save - 保存器配置字段](#save---保存器配置字�?
...
| 格式类型                             | 说明 | 示例 |
|----------------------------------|------|------|
| 短名�?                             | lab_calib目录下的数据集名�?| `"calibImages"` |
| 纯图像目�?                           | 包含图像文件的目录路径（支持相对路径和绝对路径） | `"/path/to/images"` �?`"./images"` |
| 包含JSON/JSONL文件描述自定义文本prompt的图像目�?| 包含图像文件和JSON/JSONL文件（用于描述每个图像的自定义文本prompt）的目录路径（支持相对路径和绝对路径�?| `"/path/to/images"` �?`"./images"` |
...
```yaml
spec:
  dataset: "/path/to/images"   # 使用绝对路径的图像目�?
```
...
```yaml
spec:
  dataset: "/path/to/images_with_json"  # 使用包含JSON/JSONL描述文件的图像目�?
```
...
- **Iterative Smooth**：首选方案。运行快，精度较高，适用于绝大多�?W8A8 场景�?
- **Flex Smooth Quant**：进阶方案。支持自动参数搜索，�?Iterative Smooth 效果不佳时尝试�?
- **Flex AWQ SSZ**：低比特必备。专�?INT4/W4A8 等场景设计，使用真实量化器评估误差，精度提升显著�?
