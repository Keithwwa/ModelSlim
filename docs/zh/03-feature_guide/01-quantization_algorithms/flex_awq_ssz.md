...
- **核心思想**：Flex AWQ SSZ算法的核心思想是通过实际量化器（LinearQuantizer）来评估不同alpha参数下的量化误差，自动搜索使量化误差最小的最优alpha参数。算法固定beta�?，激活值尺度计算使用均值（mean）而非最大值（max），从而在不同量化场景下获得精度与量化效率的平衡�?
...
**关键特性：**
1. **实际量化器评�?*：与Flex Smooth Quant不同，Flex AWQ SSZ使用真实的量化器（LinearQuantizer）来评估不同alpha参数下的量化误差，而不是简单的模拟量化�?
2. **激活尺度计�?*：使用激活值的均值（mean）而非最大值（max）来计算激活尺度，更适合低bit量化场景�?
3. **Beta固定�?**：算法固定beta�?，简化参数搜索空间，专注于alpha参数的优化�?
4. **自动参数搜索**：如果未提供alpha参数，算法会在[0.0, 1.0]范围内以0.05为步长搜索最优alpha，选择使量化误差（MSE）最小的参数�?

**Alpha参数搜索流程**

1. **初始�?*：创建FlexAWQSSZAlphaBetaSearcher，使用配置的qconfig�?
...
算法�?`msmodelslim/processor/anti_outlier/flex_smooth/processor.py` 中实现，处理流程分两阶段�?
...
- 钩子�?`[batch, seq, hidden_dim]` 维度上收集激活值统计信息：
  - **激活张量数�?*：收集完整的激活张量，用于后续平滑计算�?
  - **使用第一个linear的激活统计信�?*：Flex AWQ SSZ使用子图targets中第一个线性层的激活统计信息�?
...
```yaml
- type: "flex_awq_ssz"              # 固定�?`flex_awq_ssz`，用于指�?Processor�?
  alpha: 0.8                       # 激活缩放的系数，取值范围为0-1之间，默认值为None（自动搜索），也支持用户自行配置�?
  qconfig:                         # 量化配置，为必填参数�?
    act:                           # 激活值量化配置�?
      scope: "per_token"           # 量化范围：per_token �?per_tensor�?
      dtype: "int8"                # 量化数据类型：int8�?
      symmetric: True              # 是否对称量化：True �?False�?
      method: "minmax"            # 量化方法：minmax 或其他方法�?
    weight:                        # 权重量化配置�?
      scope: "per_channel"         # 量化范围：per_channel�?
      dtype: "int4"                # 量化数据类型：int4 �?int8�?
...
| 字段�?| 作用 | 说明 |
|--------|------|------|
| type | 处理器类型标�?| 固定�?flex_awq_ssz"，用于标识这是一个灵活激活感知权重量化平滑处理器�?|
| alpha | 激活缩放权重系�?| 0-1之间的浮点数，控制激活对缩放因子的影响程度，默认None（自动搜索）�?|
| qconfig | 量化配置 | 必填参数，包含激活值（act）和权重（weight）的量化配置，用于实际量化器评估�?|
| qconfig.act | 激活值量化配�?| 包含scope、dtype、symmetric、method等字段，指定激活值的量化方式�?|
...
class MappingConfig:
    """模块映射关系配置"""
    source: str  # 源模块名称，�?"model.layers.0.input_layernorm"
...
**前置要求�?*
- 模型需要继�?`FlexSmoothQuantInterface` 接口（与Flex Smooth Quant相同）�?
- 模块名称必须�?`named_modules()` 返回的完整路径一致�?
...
**步骤�?*
1. **继承接口**：模型适配器继�?`FlexSmoothQuantInterface` 接口，实�?`get_adapter_config_for_subgraph()` 方法�?
2. **配置子图映射**：为每层配置四种类型的子图映射关系：
   - **Norm-Linear子图**：归一化层到后续线性层的映�?
   - **OV子图**：注意力机制中V投影到O投影的映�?
   - **Up-Down子图**：MLP门控机制中上投影到下投影的映�?
   - **Linear-Linear子图**：连续线性层的映�?
...
### 6. 映射关系错误
**现象**: `MappingConfig` 中的 `source` �?`targets` 指向错误的模块�? 
**解决方案**: 检�?`MappingConfig` 中的 `source` �?`targets` 是否指向正确的模块�?
