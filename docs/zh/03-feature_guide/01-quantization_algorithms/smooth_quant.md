# SmoothQuant：离群值抑制算法说�?

## 简�?

- **来源**：MIT 提出�?SmoothQuant 算法�?
...
SmoothQuant 仅支�?NormLinearSubgraph（归一�?线性子图）类型�?
...
- 对每个线性层执行正向缩放操作（权重乘�?scales�?
- 对归一化层执行反向缩放操作（权重除�?scales�?
- 如果启用非对称量化，还会计算并应用偏移量

### 实现

算法�?`msmodelslim/processor/anti_outlier/smooth_quant/` 中实现，处理流程分两阶段�?

#### 1) 预处理阶段（preprocess�?

**子图发现与构建：**
- 通过模型适配器的 `get_adapter_config_for_subgraph()` 获取子图信息�?
- 仅处�?`norm-linear` 类型的子图，其他类型会被自动过滤�?
...
- **模型架构要求**：模型必须支�?`SmoothQuantInterface` 接口，并正确配置子图映射关系�?
- **模块命名要求**：模块名称必须与 `named_modules()` 返回的完整路径完全一致�?
- **子图类型支持**：SmoothQuant 仅支�?`norm-linear` 子图类型�?
- **模块属性要�?*：目标模块必须存在且具备可写�?`weight`（以及可�?`bias`）�?
...
```yaml
- type: "smooth_quant"                    # 固定�?`smooth_quant`，用于指�?Processor�?
  alpha: 0.5                              # 浮点�? 0~1, 默认 0.5，平衡参数，控制激活和权重的相对重要性�?
...
**注意**：SmoothQuant 仅支�?`norm-linear` 子图类型，不支持其他子图类型（如 `ov`、`up-down`、`linear-linear`），因而不支持指定 `enable_subgraph_type` 字段�?
...
| 字段�?| 作用      | 说明 |
|--------|---------|------|
| type | 处理器类型标�?| 固定�?smooth_quant"，用于标识这是一个SmoothQuant处理器。|
...
class AdapterConfig:
    """子图适配器配�?""
    subgraph_type: str  # 子图类型，SmoothQuant仅支�?"norm-linear"
...
class SmoothQuantInterface(ABC):
    ...
    def get_adapter_config_for_subgraph(self) -> List[AdapterConfig]:
        """
        返回模型中所有可进行SmoothQuant处理的子图配�?
        
        Returns:
            List[AdapterConfig]: 子图配置列表，每个配置包含：
                - subgraph_type: 子图类型（应�?norm-linear"�?
...
**前置要求�?*
- 模型需要继�?`SmoothQuantInterface` 接口�?
- 模块名称必须�?`named_modules()` 返回的完整路径一致�?
- SmoothQuant 仅支�?`norm-linear` 子图类型�?
- 配置中的`subgraph_type`、`mapping` 是必要参数�?

**步骤�?*
1. **继承接口**：模型适配器继�?`SmoothQuantInterface` 接口，实�?`get_adapter_config_for_subgraph()` 方法�?
...
**解决方案**: 核对完整模块名称是否�?`named_modules()` 返回的路径一致�?
...
**解决方案**: 检�?`MappingConfig` 中的 `source` 是否为归一化层，`targets` 是否为其后续的线性层�?
