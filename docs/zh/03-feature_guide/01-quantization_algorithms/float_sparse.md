...
2. **激活统�?*：通过前向hook收集激活统计信息，构建Hessian矩阵�?
3. **迭代稀�?*：通过多次迭代逐步优化稀疏模式，平衡稀疏率和模型精度�?
...
- 算法�?`msmodelslim/processor/sparse/float_sparse.py` �?`admm.py` 中实现：
...
- **高压缩需�?*：适用于需要高压缩率的模型部署场景�?
- **精度敏感**：通过精度保护机制，在压缩的同时保持关键权重精度�?
- **计算成本**：ADMM算法需要多次迭代，计算成本较高，速度较慢�?
- **内存需�?*：需要存储Hessian矩阵和激活统计信息，显存占用较高�?
- **使用限制**�?
  - 当前算法生成的权重需要在 Atlas 300I Duo 推理卡上利用硬件压缩单元进行进一步压缩，才能有效减小模型大小，降低模型部署时的显存占用，并获得性能提升�?
  - 由于 Atlas 300I Duo 推理�?不支�?bfloat 数据类型，因此对模型进行浮点稀疏时，需要手动将模型路径下的 config.json 中的 `torch_dtype` 字段修改�?float16�?
  - 仅支�?`v1 框架` 中的 `逐层量化`�?
  - 目前仅支�?`nn.Linear` 模块进行浮点稀疏�?
  - 需要校准数据集收集激活统计信息，校准数据�?token id 个数 >= 2048�?
  - 稀疏比例建议在 `0.3` 附近逐步调整�?
...
```yaml
- type: "float_sparse"
      sparse_ratio: 0.3          # 稀疏比例，取值范围为 0.0~1.0，默�?.3�?
      include: [ "*" ]           # 包含的层，支持通配符�?
...
| 字段�?| 作用 | 数据类型 | 默认�?| 说明 |
|--------|------|----------|--------|------|
| type | 处理器类型标�?| string | - | 固定�?float_sparse"，用于标识该对象为浮点稀疏量化处理器�?|
| sparse_ratio | 稀疏比�?| float | 0.3 | 稀疏比例，取值范围为 0.0~1.0，默�?.3�?|
| include | 包含的层 | array[string] | ["*"] | 支持通配符匹配，指定要执行浮点稀疏量化的层�?|
| exclude | 排除的层 | array[string] | [] | 支持通配符匹配，优先级高于include�?|


## 模型适配
请参考[自主量化/模型接入](../../05-developer_guide/integrating_models.md)完成模型接入，在 YAML 文件中完成对应配置即可实现浮点稀疏的适配�?
...
**解决方案**：降�?sparse_ratio 配置参数，建议在 0.3 附近逐步调整�?
